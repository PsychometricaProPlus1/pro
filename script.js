...
function showAlert(type, message) { // DEFINED GLOBALLY NOW
    ...
    // Ensure body exists before inserting (though unlikely to be an issue if called after DOM load)
    if(document.body) {
        document.body.insertBefore(alertDiv, document.body.firstChild);
    } else {
        // Fallback if somehow called before body exists (very unlikely for user actions)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========================================================================
    // Utility Functions (Defined within DOMContentLoaded if they interact heavily with DOM/State)
    // ========================================================================
    // NOTE: showAlert is now defined GLOBALLY above.
    ...
    function loadInfoStep(idx) { const d = document.getElementById('info-step'); const bB = document.getElementById('info-back-btn'); const nB = document.getElementById('info-next-btn'); if (!d || !bB || !nB) return; const f = infoFields[idx]; const isM = selectedLanguage === 'marathi'; d.innerHTML = `<div class="form-group"><label for="<span class="math-inline">\{f\.id\}"\></span>{isM ? f.labelMr : f.labelEn}:</label>${f.type === 'select' ? `<select id="${f.id}" ${f.id === 'grade' ? 'disabled' : ''}>${f.options.map(o => `<option value="${o.value}" <span class="math-inline">\{o\.value \=\=\= studentData\[f\.id\] ? 'selected' \: ''\}\></span>{isM ? o.textMr : o.textEn}</option>`).join('')}</select>` : `<input type="${f.type}" id="${f.id}" placeholder="${isM ? f.labelMr : f.labelEn}" value="${studentData[f.id] || ''}" ${f.id === 'grade' ? 'readonly' : ''} ${f.type === 'number' ? 'min="' + (f.id === 'age' ? '10' : '0') + '" max="' + (f.id === 'age' ? '18' : '100') + '"' : ''}>`}</div>`; if (f.id === 'grade') { d.innerHTML = `<div class="form-group"><label><span class="math-inline">\{isM ? f\.labelMr \: f\.labelEn\}\:</label\><p style\="padding\: 10px; border\: 1px solid \#ddd; background\-color\: \#eee; border\-radius\: 8px;"\></span>{studentData.grade}</p><input type="hidden" id="grade" value="${studentData.grade}"></div>`; } else if (f.id === 'board' && studentData[f.id]) { document.getElementById(f.id).value = studentData[f.id]; } bB.style.display = idx === 0 ? 'none' : 'inline-block'; nB.textContent = idx === infoFields.length - 1 ? (isM ? 'सबमिट' : 'Submit') : (isM ? 'पुढे' : 'Next'); }
    ...
    function loadQuestion(idx) { const qs = parseInt(selectedStandard) <= 8 ? window.questions5to8?.[selectedLanguage] : window.questions9to10?.[selectedLanguage]; if (!qs || qs.length === 0) { resetUI(); return; } const q = qs[idx]; const d = document.getElementById('questions'); if (!q || !d) return; d.innerHTML = `<div class="question"><p>${idx + 1}. <span class="math-inline">\{q\.text\}</p\><div class\="options"\></span>{q.options.map((o, i) => `<label><input type="radio" name="q${idx}" value="${o.value || o}" ${userAnswers[idx] === (o.value || o) ? 'checked' : ''}><span>${o.text || o}</span></label>`).join('')}</div></div>`; updateProgressAndButtons(idx, qs.length); }
...
    // --- Admin Section Logic (Unchanged) ---
    function showAdminDashboard() { console.log('Showing admin.'); const aS = document.getElementById('admin-section'); const rT = document.querySelector('#results-table tbody'); const sT = document.querySelector('#student-info-table tbody'); if (!aS || !rT || !sT) { showAlert('error', 'Admin UI missing.'); return; } if (allResults.length > 0 || allStudentInfo.length > 0) { showAlert( 'critical-warning', 'Local data detected. Export regularly.' ); } else { showAlert('info', 'No local data.'); } aS.classList.remove('hidden'); rT.innerHTML = allResults.map(e => `<tr><td><span class="math-inline">\{e\.studentData?\.\['student\-name'\] \|\| ''\}</td\><td\></span>{e.standard || ''}</td><td><span class="math-inline">\{e\.date \|\| ''\}</td\><td\></span>{e.summary || ''}</td></tr>`).join(''); sT.innerHTML = allStudentInfo.map(i => `<tr><td><span class="math-inline">\{i\.studentName \|\| ''\}</td\><td\></span>{i.parentName || ''}</td><td><span class="math-inline">\{i\.mobile \|\| ''\}</td\><td\></span>{i.email || ''}</td><td><span class="math-inline">\{i\.school \|\| ''\}</td\><td\></span>{i.age || ''}</td><td><span class="math-inline">\{i\.board \|\| ''\}</td\><td\></span>{i.standard || ''}</td><td>${i.medium || ''}</td></tr>`).join(''); updateBrandingThroughout(); }
...
    // Assign functions used by HTML onclick attributes to window scope
    window.login = login;
...
    // NOTE: showAlert is defined globally above, no need to assign here.
    // NOTE: generateDevelopmentPlan and copyPlan are defined in plan.js and assigned to window there.
